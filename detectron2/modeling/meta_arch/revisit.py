...
class RevisitRCNN(nn.Module):
    """
    Revisit R-CNN. Any models that contains the following three components:
    1. Per-image feature extraction (aka backbone)
    2. Region proposal generation
    3. Task-aware spatial disentangled region proposal generation
    4. Per-region feature extraction and prediction, respectively.
    5. Loss tuning with progressive constraint
    """
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        if do_postprocess:
            return RevisitRCNN._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
...
@META_ARCH_REGISTRY.register()
class DisentangleRegressionNetwork(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.device = torch.device(cfg.MODEL.DEVICE)
...
@META_ARCH_REGISTRY.register()
class DisentangleClassificationNetwork(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.device = torch.device(cfg.MODEL.DEVICE)
